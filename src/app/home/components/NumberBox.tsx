...
const NumberBox = ({ num, unit, flip, countdownOver }: numProp) => {
  const shouldFlip = countdownOver && flip;
  ...
  return (
    <div className="flex flex-col items-center mt-4 px-2">
      <div className={`relative bg-transparent flex flex-col items-center justify-center rounded-lg w-32 h-32 text-2xl md:text-4xl mt-4 ${shouldFlip ? 'flip' : ''}`}>
        ...
        <div className={`text-5xl absolute ${shouldFlip ? 'text-blue-500' : countdownOver ? 'text-blue-500' : 'text-blue-500'} z-10 font-bold font-redhat md:text-7xl font-mono`}>
          {num}
        </div>
